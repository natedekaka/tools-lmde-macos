...
class SimpleButton extends WorkspaceButton {
    constructor(index, applet) {
        ...
        // Replace numeric label with a circle (radio-symbolic icon)
        this.icon = new St.Icon({
            icon_name: "radio-symbolic",  // built-in GNOME symbolic circle
            icon_size: 12
        });
    ...
    activate(active) {
        if (active) {
            this.icon.set_icon_name("media-record-symbolic"); // filled circle
        } else {
            this.icon.set_icon_name("radio-symbolic"); // empty circle
            this.update();
        }
    }
    ...
    shade(used) {
        if (!used) {
            this.icon.set_opacity(80); // dim unused workspace
        } else {
